...
@interface CollectDetailVC ()

@end

@implementation CollectDetailVC
...
- (void) creatDetailsViewController{
    ...
    [_bigPicImageView setImageWithURL:[NSURL URLWithString:self.object.picUrl]];
    ...
    _nameLabel.text = self.object.title;
    ...
    _priceLabel.text = self.object.price;
    ...
    {
        if ([model.title isEqualToString:self.object.title])
        {
    ...
    _collectFigureLabel.text = self.object.countLike;
...
- (void) heartButtonClicked:(UIButton *)button
{
    ...
    //存到收藏数据库
    [CommonTools insertCoreDataWithTitle:self.object.title Countlike:self.object.countLike PicUrl:self.object.picUrl TaobaoUrl:self.object.taobaoUrl Price:self.object.price];
}
#pragma mark - 分享
- (void) shareButtonClicked:(UIButton *)button
{
    [UMSocialSnsService presentSnsIconSheetView:self appKey:@"55ba2940e0f55af3c20026d8" shareText:self.object.title shareImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:self.object.picUrl]]] shareToSnsNames:[NSArray arrayWithObjects:UMShareToSina,UMShareToTencent,UMShareToRenren,UMShareToDouban,nil] delegate:nil];
...
- (void) taoBaoDetailsButtonClicked:(UIButton *)button{
    ...
    //存入历史数据库
    [CommonTools insertHistoryDataWithTitle:self.object.title Countlike:self.object.countLike PicUrl:self.object.picUrl TaobaoUrl:self.object.taobaoUrl Price:self.object.price];
}
@end
