...
// 活动栏  从下面 上来的 一个 AlterView
//    UIActionSheet *shat = [UIActionSheet alloc] initWithTitle:<#(NSString *)#> delegate:<#(id<UIActionSheetDelegate>)#> cancelButtonTitle:<#(NSString *)#> destructiveButtonTitle:<#(NSString *)#> otherButtonTitles:<#(NSString *), ...#>, nil
- (void)viewDidLoad
{
    [super viewDidLoad];
...
//TODO: 大图片
    ...
    [_bigPicImageView setImageWithURL:[NSURL URLWithString:self.typeH.bigPicUrl]];
    
    
//    [_bigPicImageView sd_setImageWithURL:[NSURL URLWithString:self.typeH.bigPicUrl] placeholderImage:[UIImage imageNamed:@"商品默认图片"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
//    }];
    [self.view addSubview:_bigPicImageView];
//TODO:名字Label
    _nameLabel = [[UILabel alloc] init];
    _nameLabel.numberOfLines = 0;
    _nameLabel.text = self.typeH.title;
...
//TODO:价钱Label
    ...
    _priceLabel.text = self.typeH.price;
...
//TODO:心形Button
    ...
    _heartButton.frame = CGRectMake(x + 3, heartY, 25, 25);
    ...
    {
        if ([model.title isEqualToString:self.typeH.title])
        {
...
//TODO:收藏数字Label
    ...
    _collectFigureLabel.text = self.typeH.countLike;
    _collectFigureLabel.textColor = [UIColor colorWithRed:254.0/255.0f green:73.0/255.0f blue:220.0/255.0f alpha:1];
//    _collectFigureLabel.text = [NSString stringWithFormat:@"%@人喜欢",self.typeH.countLike];
...
- (void) heartButtonClicked:(UIButton *)button
{
    ...
    //存到收藏数据库
    [CommonTools insertCoreDataWithTitle:self.typeH.title Countlike:self.typeH.countLike PicUrl:self.typeH.picUrl TaobaoUrl:self.typeH.taobaoUrl Price:self.typeH.price];
}
#pragma mark - 分享
- (void) shareButtonClicked:(UIButton *)button
{
    [UMSocialSnsService presentSnsIconSheetView:self appKey:@"55ba2940e0f55af3c20026d8" shareText:self.typeH.title shareImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:self.typeH.bigPicUrl]]] shareToSnsNames:[NSArray arrayWithObjects:UMShareToSina,UMShareToTencent,UMShareToRenren,UMShareToDouban,nil] delegate:nil];
...
- (void) taoBaoDetailsButtonClicked:(UIButton *)button{
    ...
    //存入历史数据库
    [CommonTools insertHistoryDataWithTitle:self.typeH.title Countlike:self.typeH.countLike PicUrl:self.typeH.picUrl TaobaoUrl:self.typeH.taobaoUrl Price:self.typeH.price];
}
@end
